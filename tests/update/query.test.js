...
describe("query function", () => {
  ...
  it("should generate a valid SQL UPDATE query without consumerId and consumerName", () => {
    ...
    expect(query).toBe(`UPDATE \"consumer\" SET \"accessToken\" = CASE WHEN id = $1 THEN $3 WHEN id = $2 THEN $4 END, \"refreshToken\" = CASE WHEN id = $1 THEN $5 WHEN id = $2 THEN $6 END WHERE id IN ($1, $2)`);
    expect(args).toEqual([
  ...
  it("should generate a valid SQL UPDATE query without consumerId and consumerName and one row", () => {
    ...
    const { query, args } = consumerEntity.query.update(chunk);
    expect(query).toBe(`UPDATE \"consumer\" SET \"accessToken\" = CASE WHEN id = $1 THEN $2 END, \"refreshToken\" = CASE WHEN id = $1 THEN $3 END WHERE id IN ($1)`);
    expect(args).toEqual([
      1, 'Jxxxx', 'fdjlz'
    ]);
  });
